...
## 2. Softwaredesign
#### Übersichtsdiagramm
![[Pasted image 20230522140836.png]]

#### Verteilungsdiagramm
![[Pasted image 20230522144406.png]]
...
![[Pasted image 20230521005016.png]]

<hr>
_**Die SpringBoot-Applikation sieht folgendermaßen aus:
1. Die Main-Klasse. Sie ist die Klasse, die ausgeführt wird.
![[Pasted image 20230520230436.png]]
...
2. Die `ToDo`-Klasse. Darin befinden sich die benötigten Attribute, die ein ToDo-Objekt braucht.
   ![[Pasted image 20230520231140.png]]

3. Die `ToDoController`-Klasse. Diese Klasse ist quasi das Herzstück der Spring-Applikation und  nimmt Anfragen von den Clients entgegen, verarbeitet diese und sendet die entsprechende Antwort zurück. Diese Klasse ist der Vermittler zwischen Client und der Anwendung und steuert den Ablauf der Abfragen. ![[Pasted image 20230520231651.png]]
   ...
   Der GET-Endpoint:![[Pasted image 20230520233050.png]]
   ...
   Der POST-Endpoint:![[Pasted image 20230520233842.png]]
   ...
   Der PUT-Endpoint: ![[Pasted image 20230520234303.png]]
   ...
   Der DELETE-Endpoint: ![[Pasted image 20230521000408.png]]
...
4. Das `ToDoRepository`-Interface. Das Repository-Interface steht für eine Komponente, die für den Zugriff auf eine Datenbank oder eine andere Datenquelle verantwortlich ist. Sie stellt eine Abstraktionsschicht zwischen der Datenbank und der restlichen Anwendung bereit. ![[Pasted image 20230521002559.png]]
...
5. Die `application.properties`-Datei ist eine Konfigurationsdatei, inder man Einstellungen und Eigenschaften für die Anwendung festlegt. Beispiele für Konfigurationsoptionen sind Serverkonfigurationen (z.B. den Server-Port) oder Datenbankunfigurationen (Verbindung zur Datenbank). ![[Pasted image 20230521010931.png]]
...
![[Pasted image 20230521161429.png]]
Die obigen Dependencies werden benötigt, damit MongoDB, aber auch SpringBoot gemeinsam funktionieren. 
...
Die HTTP-Methoden in React sind mit dem HTTP-Client axios realisiert worden. Damit man React verstehen kann muss man wissen was Hooks sind: React-Hooks erlauben es Funktionen auf den aktuellen State und auf andere React-Komponenten zuzugreifen. Wegen diesn Hooks werden in React keine Klassen benötigt

1. GET-Methode ![[Pasted image 20230521184924.png]]
...
2. POST-Methode    ![[Pasted image 20230521185242.png]]
...
3. PUT-Methode ![[Pasted image 20230521192436.png]]
   Die meisten Sachen unterscheiden sich nicht von der POST-Methode. Der `useParams`-Hook erlaubt den Zugriff auf den Wert diese Parameters, in diesem Fall `id`, wenn in der Route eine dynamische Parameterplatzhalter festgelegt hast, z. B. `"/todos/:id"`. Das würde so aussehen:![[Pasted image 20230521192909.png]]
...
4. Die DELETE-Methode ![[Pasted image 20230521200324.png]]
  Die `deleteTodo`-Funktion löscht das Objekt mir der entsprechenden `id` mit `axios.delete` und wird dann ausgeführt wenn der Löschknopf gedrückt wird:![[Pasted image 20230521200720.png]]
...
React Router wird dazu verwendet um via URL zu einer anderen Seite zu gelangen. Beispielsweise kann man von der `App.js` Datei zur `Home.js`-Datei mithilfe von `/home`  gelangen: ![[Pasted image 20230521210005.png]]
...
![[Pasted image 20230522064623.png]]
...
1. GET-Methode ![[Pasted image 20230521231648.png]]
...
2. POST-Methode![[Pasted image 20230521233148.png]]
   Als erstes wird ein neues `ToDo`-Objekt erstellt um die einzelnen Attribute mit den eingegebenen Werten zu befüllen. ![[Pasted image 20230521233428.png]]
...
3. PUT-Methode![[Pasted image 20230522001519.png]]
...
4. DELETE-Methode
   ![[Pasted image 20230521234301.png]]
